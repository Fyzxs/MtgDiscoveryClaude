...
/// </summary>
internal sealed class AllSetsDomain : IAllSetsDomain
{
    private readonly ISetAggregatorService _setAggregatorService;

    public AllSetsDomain(ILogger logger) : this(new SetAggregatorService(logger))
    { }

    private AllSetsDomain(ISetAggregatorService setAggregatorService) => _setAggregatorService = setAggregatorService;
