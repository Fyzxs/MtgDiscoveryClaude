...
/// </summary>
internal sealed class SetsDomain : ISetsDomain
{
    private readonly ISetAggregatorService _setAggregatorService;

    public SetsDomain(ILogger logger) : this(new SetAggregatorService(logger))
    { }

    private SetsDomain(ISetAggregatorService setAggregatorService) => _setAggregatorService = setAggregatorService;
