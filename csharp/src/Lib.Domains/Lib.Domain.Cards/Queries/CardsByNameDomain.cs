...
/// </summary>
internal sealed class CardsByNameDomain : ICardsByNameDomain
{
    private readonly ICardAggregatorService _cardAggregatorService;

    public CardsByNameDomain(ILogger logger) : this(new CardAggregatorService(logger))
    { }

    private CardsByNameDomain(ICardAggregatorService cardAggregatorService) => _cardAggregatorService = cardAggregatorService;
