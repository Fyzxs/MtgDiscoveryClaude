...
internal sealed class UserCardBySetEnrichment : IUserCardBySetEnrichment
{
    ...
    public async Task Enrich(List<CardItemOutEntity> target, IUserCardsSetItrEntity context, CancellationToken cancellationToken)
    {
        IOperationResponse<IEnumerable<IUserCardOufEntity>> userCardResponse = await _userCardsDomainService.UserCardsBySetAsync(context, cancellationToken).ConfigureAwait(false);
