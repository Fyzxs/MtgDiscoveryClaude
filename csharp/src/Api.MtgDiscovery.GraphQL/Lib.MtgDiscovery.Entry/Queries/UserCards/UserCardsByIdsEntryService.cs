...
internal sealed class UserCardsByIdsEntryService : IUserCardsByIdsEntryService
{
    ...
    public async Task<IOperationResponse<List<UserCardOutEntity>>> Execute(IUserCardsByIdsArgEntity cardsArgs, CancellationToken cancellationToken)
    {
        ...
        IOperationResponse<IEnumerable<IUserCardOufEntity>> opResponse = await _userCardsDomainService.UserCardsByIdsAsync(itrEntity, cancellationToken).ConfigureAwait(false);
