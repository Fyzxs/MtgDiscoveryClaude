...
internal sealed class UserCardsBySetEntryService : IUserCardsBySetEntryService
{
    ...
    public async Task<IOperationResponse<List<UserCardOutEntity>>> Execute(IUserCardsBySetArgEntity bySetArgs, CancellationToken cancellationToken)
    {
        ...
        IOperationResponse<IEnumerable<IUserCardOufEntity>> opResponse = await _userCardsDomainService.UserCardsBySetAsync(itrEntity, cancellationToken).ConfigureAwait(false);
