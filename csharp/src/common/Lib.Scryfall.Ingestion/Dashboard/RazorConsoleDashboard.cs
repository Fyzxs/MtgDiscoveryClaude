...
internal sealed partial class RazorConsoleDashboard : IIngestionDashboard, IDisposable
{
    ...
    public void UpdateProgress(string type, int current, int total, string action, string item) => _state.UpdateProgress(type, current, total, action, item);

    public void UpdateSetProgress(int current, int total, string name) => _state.UpdateSetProgress(current, total, name);

    public void UpdateCardProgress(int current, int total, string name) => _state.UpdateCardProgress(current, total, name);

    public void UpdateRulingProgress(int current, int total, string name) => _state.UpdateRulingProgress(current, total, name);

    public void UpdateTrigramProgress(int current, int total, string name) => _state.UpdateTrigramProgress(current, total, name);
    ...
    public void UpdateCompletedCount(string type, int count) => _state.UpdateCompletedCount(type, count);
    ...
    public IDisposable BeginScope<TState>(TState state) where TState : notnull => _fallbackLogger.BeginScope(state);
