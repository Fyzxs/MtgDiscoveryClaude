import { useCollectionDelta } from '../contexts/CollectionContext';
 ...
 *
 * Uses the delta context (volatile) so only lightweight subscribers
 * like DeltaBadge re-render — not every MtgCard on the page.
 */
export function useLastDelta(cardId: string): number | undefined {
  const { getLastDelta, lastDeltaVersion } = useCollectionDelta();
