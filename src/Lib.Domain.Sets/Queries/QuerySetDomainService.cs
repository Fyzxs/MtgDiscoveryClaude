...
internal sealed class QuerySetDomainService : ISetDomainService
{
    ...
    private QuerySetDomainService(ISetAggregatorService setAggregatorService)
    {
        _setAggregatorService = setAggregatorService;
    }

    public async Task<IOperationResponse<ISetItemCollectionItrEntity>> SetsAsync(ISetIdsItrEntity setIds)
    {
        return await _setAggregatorService.SetsAsync(setIds).ConfigureAwait(false);
    }
